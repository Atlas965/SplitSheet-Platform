...
interface ContractData {
  ...
  collaborators?: any[];
  createdAt: string;
  /** Counsel-supplied legal body (Priority 1.2) — the exact snapshot stored
   *  on the contract's metadata at creation/signing time, not necessarily
   *  the template's current text. Rendered as its own section; version is
   *  also stamped into the PDF footer. */
...
export function generateContractPDF(contract: ContractData): jsPDF {
  ...
  // Legal terms (Priority 1.2) — counsel-supplied body, rendered as plain
  // text since jsPDF has no markdown/rich-text support.
  if (contract.legalBodyMarkdown) {
    doc.setFont(undefined, 'bold');
    doc.setFontSize(12);
    doc.text(
      contract.legalBodyVersion ? `LEGAL TERMS (v${contract.legalBodyVersion})` : 'LEGAL TERMS',
      margin,
      yPosition
    );
    yPosition += 10;
    ...
    for (const line of markdownToPlainTextLines(contract.legalBodyMarkdown)) {
      if (yPosition > 260) {
        doc.addPage();
        yPosition = margin;
      }
      ...
      yPosition = addWrappedText(line, margin, yPosition, contentWidth, 10) + 3;
    }
    yPosition += 10;
  }
  ...
  doc.setFont(undefined, 'normal');
  doc.setFontSize(10);
  const disclaimer = 'This contract was generated using SplitSheet. Please review all terms carefully and consult with a qualified attorney before signing. SplitSheet is not responsible for the legal validity or enforceability of this agreement.';
  ...
  doc.setFontSize(10);
  const footerLeft = contract.legalBodyVersion
    ? `Generated by SplitSheet · Legal terms v${contract.legalBodyVersion}`
    : 'Generated by SplitSheet - Music Agreement Management Platform';
...
export function downloadContractPDF(contract: ContractData) {
  ...
  const filename = `${contract.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_agreement.pdf`;
  doc.save(filename);
}
